...
class DocumentsController {
  ...
  async convertUrlToPdf(req: Request, res: Response) {
    const { url, namespaceId } = req.body;
    if (!url || !namespaceId) {
      return res.status(400).json({ message: 'Missing URL or namespaceId' });
    }

    const pdfPath = `/tmp/${uuidv4()}.pdf`;
    try {
      const browser = await puppeteer.launch(); // Launch a Puppeteer browser instance
      const page = await browser.newPage(); // Open a new page
      await page.goto(url, { waitUntil: 'networkidle2' }); // Navigate to the URL and wait until the network is idle
      await page.pdf({ path: pdfPath, format: 'A4' }); // Save the page as a PDF
      await browser.close(); // Close the browser

      const documentId = uuidv4(); // Generate a unique ID for the document
      const fileKey = `${namespaceId}/${documentId}/${path.basename(pdfPath)}`; // Generate the file key for storage
      const documentUrl = storageService.constructFileUrl(fileKey); // Construct the file URL

      const fileData = fs.readFileSync(pdfPath); // Read the PDF file

      // Save the file to the storage service
      await storageService.saveFile(
        { path: pdfPath, mimetype: 'application/pdf', originalname: path.basename(pdfPath) } as Express.Multer.File,
        fileKey
      );
      ...
      worker.on('message', async (result: any) => {
        if (result.error) {
          ...
          await this.safeUpsertDocument(result.document, namespaceId);
         // await index.upsert([{ id: documentId, values: result.document.embeddings }]);
          return res.status(200).json({ message: 'Document added successfully', document: result.document });
        }
      });
      ...
      console.error('Failed to convert URL to PDF:', error);
      res.status(500).json({ message: 'Failed to convert URL to PDF' });
    }
  }
